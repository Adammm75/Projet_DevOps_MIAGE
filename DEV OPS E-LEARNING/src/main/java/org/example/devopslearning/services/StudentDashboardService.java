...
public class StudentDashboardService {
        ...
        public Map<String, Object> buildDashboard(Long studentId) {
                ...
                List<Assignment> devoirs = cours.stream()
                                ...
                                .filter(a -> !submissionRepository.existsByStudentIdAndAssignmentId(studentId,
                                                a.getId()))
                                .toList();

                List<NotesCour> notes = notesCourRepository.findByEtudiantId(studentId);
                List<Message> messages = messageRepository.findTop5ByRecipientOrderBySentAtDesc(student);

                List<Notification> notifications = notificationRepository.findByUserIdAndIsReadFalse(studentId);

                // Commentaires pédagogiques visibles par l'étudiant (TEACHER_ADMIN uniquement)
                List<org.example.devopslearning.entities.TeacherComment> comments = teacherCommentRepository
                                .findVisibleByStudentId(studentId);
                long newCommentsCount = comments.stream()
                                .filter(c -> c.getCreatedAt() != null &&
                                                c.getCreatedAt().isAfter(java.time.Instant.now().minus(7,
